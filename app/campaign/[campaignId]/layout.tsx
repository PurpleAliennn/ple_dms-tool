...
export default function CampaignLayout({
  ...
  const deletePage = async (pageId: string, sectionId: string) => { const { error } = await supabase.from('pages').delete().eq('id', pageId); if (!error) setSections(sections.map(sec => sec.id === sectionId ? { ...sec, subSections: sec.subSections.filter((p: any) => p.id !== pageId) } : sec)); };
  const handleOpenModal = (type: 'category' | 'page', sectionId?: string) => { setModalConfig({ type, sectionId }); setInputValue(""); setIsModalOpen(true); };
  const handleModalSubmit = () => { if (modalConfig.type === 'category') addSection(inputValue); else if (modalConfig.type === 'page' && modalConfig.sectionId) addSubSection(modalConfig.sectionId, inputValue); setIsModalOpen(false); };
                ...
                <div className={styles.cardBody}>
                  {section.subSections.map((sub: any) => (
                    <div key={sub.id} className={styles.sidebarRow}>
                      <Link href={`/campaign/${campaignId}/${sub.id}`} className={styles.subItem}>{sub.label}</Link>
                      <button onClick={() => confirmDeletePage(sub.id, section.id)} className={styles.trashBtnSmall}>×</button>
                    </div>
                  ))}
                  <button className={styles.addPageBtn} onClick={() => handleOpenModal('page', section.id)}>+ Add Page</button>
                </div>
              )}
            </div>
          ))}
          <button className={styles.addCategoryBtn} onClick={() => handleOpenModal('category')}>+ New Category</button>
        </aside>
